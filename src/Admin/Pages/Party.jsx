...
import { Box, ListItem, ModalClose, Sheet, Typography } from '@mui/joy';
import DataTable from '../../Atoms/DataTable';
import { ADD_PARTY_PENDING, DELETE_PARTY_PENDING, FETCH_PARTIES_PENDING } from '../../redux-saga/admin/action/Action';
import { party_delete_req, party_post_req } from '../../redux-saga/Constant';
...
export default function Party() {
  ...
  useEffect(() => {
    dispatch({ type: FETCH_PARTIES_PENDING });
  }, [dispatch]);

  const data = useSelector((state) => state.adminReducer.party);

  const columns = [
    { id: 'img', label: 'Party Logo', minWidth: 170, align: 'center' },
    { id: 'PartyName', label: 'Party Name', minWidth: 170, align: 'center' },
    { id: 'PartySCode', label: 'Party Short-Code', minWidth: 170, align: 'center' },
  ];
  ...
  const handleSubmit = (event) => {
    ...
    formData.append('party_name', event.target.elements['party-name'].value);
    formData.append('party_logo', event.target.elements['party-logo'].files[0]);
    formData.append('short_code', event.target.elements['party-short-code'].value);
    dispatch({ type: ADD_PARTY_PENDING, payload: formData, endpoint: party_post_req });
    setOpen(false);
  };
  ...
  const filteredRows = rows.filter(row =>
    ...
    row.PartySCode.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <React.Fragment>
      <Grid container spacing={2} sx={{ flexGrow: 1 }}>
            ...
            <div>
              ...
              >
                ...
                >
                  ...
                  <form onSubmit={handleSubmit}>
                    <TextField id="party-name" label="Party Name" variant="outlined" fullWidth required />
                    <input type="file" id="party-logo" name="partyLogo" accept="image/*" required />
                    <TextField id="party-short-code" label="Party Short Code" variant="outlined" fullWidth required />
                    <Button type="submit" variant="solid" color="primary">
                      Submit
                    </Button>
                  </form>
        ...
        <Grid xs={12}>
          <Box mt={11}>
            <DataTable columns={columns} rows={filteredRows} onDelete={handleDelete} onUpdate={handleUpdate} height={450} />
          </Box>
        </Grid>
      </Grid>
    </React.Fragment>
